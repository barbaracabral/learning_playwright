...
const buildFakeUsuario = (overrides: Partial<Usuario> = {}): Usuario => ({
...
test.describe('Usuários - mocks de API', () => {
  test('deve listar usuários via mock GET', async ({ page }) => {
    ...
    const fakeUser = buildFakeUsuario();
  ...
  test('deve criar usuário via mock POST', async ({ page }) => {
    const usuarioAPI = new UsuarioAPIPage(page);
    const newUser: Usuario = buildFakeUsuario({
      _id: 'created-user-id',
      nome: 'Novo Usuário',
      email: 'novo.usuario@example.com'
    });

    await usuarioAPI.mockCreateUsuario(newUser);
  ...
  test('deve atualizar usuário via mock PUT', async ({ page }) => {
    const usuarioAPI = new UsuarioAPIPage(page);
    const updatedUser: Usuario = buildFakeUsuario({
      _id: 'updated-user-id',
      nome: 'Usuário Atualizado',
      email: 'atualizado@example.com'
    });

    await usuarioAPI.mockUpdateUsuario(updatedUser);
  ...
  test('deve deletar usuário via mock DELETE', async ({ page }) => {
    ...
    await usuarioAPI.mockDeleteUsuario('fake-user-id');
